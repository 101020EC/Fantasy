...
import { Shield, Search, TrendingUp, TrendingDown, Sparkles, ArrowRight, User, Send, ChevronRight, Activity } from 'lucide-react';
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] flex flex-col justify-between w-full px-3 sm:px-6 py-4 sm:py-8 max-w-5xl mx-auto">
      <main className="w-full space-y-4 sm:space-y-6">
        ...
        <div className="card-pastel-blue p-6 sm:p-8 relative overflow-hidden shadow-xl">
          <div className="flex items-center justify-between mb-4">
            <div className="w-10 h-10 rounded-full bg-white/80 flex items-center justify-center shadow-sm">
              <Shield className="w-5 h-5 text-[#38003c]" />
            </div>
            <div className="w-10 h-10 rounded-full border border-[#111318]/20 flex items-center justify-center font-black text-xs text-[#111318] bg-white/30 backdrop-blur-sm">
              GW{currentGw}
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <button
              ...
              className="px-6 py-2.5 bg-[#38003c] text-white font-black text-xs sm:text-sm rounded-full shadow-lg hover:scale-105 active:scale-95 transition-transform flex items-center gap-2"
            >
            ...
            <button
              ...
              className="px-4 py-2.5 bg-white/70 backdrop-blur-md text-[#111318] font-bold text-xs rounded-full border border-black/10 hover:bg-white transition flex items-center gap-1.5"
            >
        ...
        {/* 2. Gameweek Schedule Capsule Bar: Starting from GW 1 to GW 27 (Current) up to GW 38 */}
        <div className="pastel-card p-3 sm:p-4 shadow-sm flex items-center overflow-x-auto gap-2 scrollbar-none">
          {Array.from({ length: 38 }, (_, i) => i + 1).map((gw) => {
            ...
            return (
              <div
                key={gw}
                className={`flex-shrink-0 min-w-[65px] text-center py-2 px-1.5 rounded-full text-xs transition ${
                  isCurrent
              ...
              >
                <span className="block text-[8px] uppercase opacity-75">
                  {isCurrent ? 'Active' : isFinished ? 'Done' : 'Next'}
                </span>
                <span className="text-xs">GW {gw}</span>
              </div>
            );
          })}
        </div>
        ...
        <div className="pastel-card p-4 sm:p-6 shadow-sm">
          ...
          <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-2">
            ...
            <button
              type="submit"
              disabled={loading}
              className="px-8 py-3.5 bg-[#38003c] text-white font-black text-sm rounded-full shadow-md hover:opacity-90 active:scale-95 transition disabled:opacity-50 flex items-center justify-center gap-2"
            >
          ...
          <div className="flex items-center gap-2 mt-3 text-xs text-gray-400 flex-wrap">
            ...
            {['1', '100', '12345', '54321'].map((id) => (
              <button
                ...
                className="px-2.5 py-0.5 rounded-full bg-gray-100 hover:bg-purple-100 text-gray-700 font-mono text-[11px] transition"
              >
                #{id}
              </button>
            ))}
        ...
        {/* 4. Two Soft Pastel Cards: Lilac & Warm Orange */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="w-9 h-9 rounded-full border border-[#111318]/20 flex items-center justify-center font-black text-xs text-[#111318] bg-white/30 backdrop-blur-sm">
                +£
              </div>
            </div>

            <span className="text-[11px] font-bold text-[#111318]/70 block uppercase tracking-wider">
              Price Radar
            </span>
            <h3 className="text-xl font-black text-[#111318] mt-0.5 mb-1">
              Top Risers Alert
            </h3>
            <p className="text-xs text-[#111318]/80 leading-relaxed mb-4">
              เรดาร์เช็คยอดซื้อเข้าสุทธิ ดักซื้อก่อนนักเตะราคาแพงขึ้นคืนนี้
            </p>

            <div className="flex items-center justify-between pt-2 border-t border-black/10 text-xs font-black text-[#111318]">
        ...
        <div className="space-y-2.5">
          ...
          <button
            onClick={handleOpenMyTeam}
            className="w-full p-4 rounded-full bg-[#38003c] text-white flex items-center justify-between hover:scale-[1.01] active:scale-98 transition shadow-md"
          >
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-full bg-white text-[#38003c] flex items-center justify-center font-bold">
                <Shield className="w-4 h-4" />
              </div>
              <div className="text-left">
                <div className="font-bold text-sm">Squad Formation (Pitch View)</div>
                <div className="text-[11px] text-purple-200">11 ตัวจริง และตัวสำรอง</div>
              </div>
            </div>
          ...
          <Link
            href="/prices"
            className="w-full p-4 rounded-full bg-pastel-blueLight text-[#111318] flex items-center justify-between hover:scale-[1.01] active:scale-98 transition shadow-sm block"
          >
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-full bg-[#111318] text-white flex items-center justify-center font-bold">
                <Activity className="w-4 h-4" />
              </div>
              <div className="text-left">
                <div className="font-bold text-sm">Full Premier League Market</div>
                <div className="text-[11px] text-gray-600">ตลาดราคาผู้เล่นทุกคนในลีก</div>
              </div>
            </div>
      ...
      {/* Footer */}
      <footer className="border-t border-black/5 py-5 text-center text-xs text-gray-400 mt-8">
        <p>FPL Radar &bull; Powered by Next.js &amp; Firebase</p>
      </footer>
